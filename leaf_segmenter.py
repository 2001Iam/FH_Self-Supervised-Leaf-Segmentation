...
for i in range(1, 34):
    def parse_args():
        ...
        parser.add_argument('--input', type=str, default=f'./file2/kimg_{i}.png', help='input image path')
    ...
    if __name__ == "__main__":
        ...
        if os.path.exists(args.ccm):
            ...
            print('Color-corrected image has been saved to {}'.format(
                os.path.join(args.save_path, "color_corrected.jpg")))
        else:
            img = cv2.imread(args.input)

        img = imutils.resize(img, height=480, width=640)
        # img = imutils.resize(img, width=512)
        cv2.imwrite(f'modify_Input_num80/kimg_{i}.jpg',img)
        ...
        for batch_idx in pbar:
            ...
            pbar.set_description(
                "Iterations {0}/{1}: {2}, {3:.2f}".format(batch_idx, args.max_iter, num_labels, loss.item()))
        ...
        if args.save_video:
            ...
            # save_result_img(save_result_path, rgb_image, labels, combine,

            # absolute_greenness, relative_greenness_gray, thresholded)
